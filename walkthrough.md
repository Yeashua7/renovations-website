...
index.html
 (Home/Hub): Serves as the master entry point and controller. It features the primary Hero section, trust statistic counts, a welcome overview, a 3-service teaser grid (linking to the full grid), and a high-impact call-to-action.
services.html
 (Services Grid): Displays all 5 renovation specialties (Whole-Home, Kitchen, Bathroom, General Contracting, Decks/Porches) in an interactive card grid. Clicking any specialty opens a descriptive modal loaded with code-compliant specifications.
about.html
 (Our Story & Benefits): Contains the company story, founder biography card, trust counters, and a dynamic, auto-sliding benefit tabs system (Craftsmanship, Communication, Custom Solutions, Local Experience, Full-Service).
contact.html
 (Lead Capture & Map): Integrates office details, operational hours, state license credentials (MHIC, VA, DC), a visual area map mockup, and a lead capture form with custom validation, loading spinner, and success message overlay.
